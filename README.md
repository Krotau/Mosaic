...
Every successful HTTP response includes an opaque `X-Request-ID` generated by Mosaic. The
same value appears in the structured `http_request_completed` record so an operator can
correlate the response with its log. Phase 0 records the request actor as `anonymous`; it
does not accept caller-supplied identity or perform authentication or authorization.

## Configuration

Configuration is read once when the application is constructed. Mosaic loads `.env` from
the process working directory when that file exists, and real environment variables
override its values. Copying `.env.example` to `.env` therefore creates the documented
non-secret local configuration without changing source code.
